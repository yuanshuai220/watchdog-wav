		...
		for parentdir, dirname, filenames in os.walk(audio_dir):
			for filename in filenames:
				...
				if position[1] == '.wav':
					...
					cursor.execute("insert into annotator_audio (Audio_creat_time, Audio_address, Audio_name, Audio_des, Audio_length, Audio_size, Audio_format) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (Audio_create_time, Audio_address, Audio_name, '', Audio_length, Audio_size, Audio_format))
